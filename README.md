...
- **Americas** (`Americas`) — World Climbing Pan America circuit (Series / Championships) from the same content API, plus the North American Cup Series (NACS / “America Cup”) from [usaclimbing.org](https://usaclimbing.org/compete/north-american-cup-series/). Bundled JSON, refreshed by re-running the scraper.
