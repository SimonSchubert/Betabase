...
- **Americas** (`Americas`) — World Climbing events hosted in the Americas (Pan America Series/Championships plus World Series / Olympics stops in the region) from the official content API, plus the North American Cup Series (NACS / “America Cup”) and dated USA Climbing Elite Series stops from [usaclimbing.org](https://usaclimbing.org/compete/north-american-cup-series/). Bundled JSON, refreshed by re-running the scraper.
...
- **SCA** (`Australia`) — Sport Climbing Australia upcoming events from `sportclimbingaustralia.org.au/events`. Bundled JSON, same refresh model as NKBV.
...
- Region toggle chips (`World` / `Europe` / `Americas` / `Netherlands` / `Australia` / `Germany`) — all on by default.
...
python3 scripts/scrape_americas.py
